...
# %% auto 0
__all__ = ["transform_to_images", "plot_ts2img", "transform_ts2img_tensor"]
...
# %% ../../nbs/src/tsfeatures.image.ipynb 10
def transform_ts2img_tensor(df, label_col, data_dir="processed_data", categorical_label=False):
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Transforming & Saving (X, y)"):
        img_tensor = transform_to_images(row)  # Ensure this returns a torch tensor
        sample = {"image": img_tensor, "label": torch.tensor(labels[row.name], dtype=torch.long)}
